...
### `cst rm <id> [<id> ...]` — 세션 트랜스크립트 삭제

```bash
cst rm 960faaa8 --dry-run       # 무엇이 삭제될지 표시
cst rm 960faaa8 -y              # 확인 없이 삭제
cst rm 960faaa8 3a7f21bc -y     # 한 번에 여러 id 삭제
```
...
`--before`는 서로 배타적이며, `--days N`은 (`cst list`와 동일하게) "최근
N일"을 뜻합니다 — "N일 이상 지난"이 아닙니다. 이는 `cst backup --days N`과
...
- **도움말 (`?`)** — 스크롤 가능한 치트시트
- **미리보기 (`v`)** — 역할별 색상의 트랜스크립트, 메시지 전문 표시(줄바꿈 처리). 상단 메타 정보(Session / Status / Cwd / Branch / Started)는 구분선 위에 고정되어 스크롤해도 사라지지 않는다; `d`/`Ctrl-D`로 done 토글(스크롤 위치와 검색 상태 유지), `Del`로 그 자리에서 삭제(확인)
